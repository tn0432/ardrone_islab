...
public:
  ...
  CommunicationToDrone()
  {
    ...
    sub_ = n_.subscribe("ardrone_1/pose", 1, &CommunicationToDrone::Pose_Callback, this);
  ...
  void Pose_Callback(const geometry_msgs::Pose& msgPose) // Trajectory Control
  {
    //n_.getParam("/mode", modeset);
    //printf("mode [%s] \n",modeset.c_str()); 
    if (n_.getParam("/mode", modeset))
    ...
    { 
   	...
   	tf::StampedTransform transform;
    	try{
      		listener.lookupTransform("/ardrone_1/base_link","/goal",   
	...
	geometry_msgs::Twist twistframes;
        try{
 		listener.lookupTwist("/ardrone_1/base_link","/goal",   
